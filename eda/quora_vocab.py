...
class CommentVocab(object):
    ...
    def _vocab_counts_(self, comments):
        ...
        # Token Processing
        for _, comment, label in ProgressBar(comments,
        ...
        self.label_probs = [
            self.label_counts[i] / sum(self.label_counts) for i in [0, 1]
        ]

        # Uni/Bi/Trigram Totals
        self.unigram_totals = [
            sum(self.unigram_counts[i].values()) for i in [0, 1]
        ]
        self.bigram_totals = [
            sum([
                sum(endgram_counts.values())
    ...
    def _additive_smoothing_(self,
        ...
        """
        def smooth_freq(gram_count, gram_total_count, vocab_count, counts,
                        param, logs):
            """
            ...
            if counts:
                ...
                if logs:
                    count_or_freq = np.log(
                        (param + gram_count) /
                        (param * vocab_count + gram_total_count))
                else:
                    count_or_freq = (param + gram_count) / (
                        param * vocab_count + gram_total_count)
        ...
        if gram_length == 1:
            ...
            gram_frequency = [{
                gram: smooth_freq(gram_count, self.unigram_totals[i],
            ...
            for i in [0, 1]:
                gram_frequency[i].update({
                    '<unk>':
                    smooth_freq(1, self.unigram_totals[i], vocab_size[i],
                                counts, param, logs)
                })
        ...
        if gram_length == 2:
            vocab_size = [
                sum([
                    len(endgram_counts.keys())
                    for endgram_counts in self.bigram_counts[i].values()
                ]) for i in [0, 1]
            ]
            gram_frequency = [[{
                '_'.join([gram1, gram2]):
                smooth_freq(gram_count, self.bigram_totals[i], vocab_size[i],
                            counts, param, logs)
                for gram2, gram_count in self.bigram_counts[i].get(
                    gram1).items()
            } for gram1 in self.bigram_counts[i].keys()] for i in [0, 1]]
            # Flattening list of dicts into one dict
            gram_frequency = [{
                k: v
                for d in gram_frequency[i] for k, v in d.items()
            } for i in [0, 1]]
            # Adding smoothed values for words not in training vocab
            for i in [0, 1]:
                gram_frequency[i].update({
                    '<unk>':
                    smooth_freq(1, self.bigram_totals[i], vocab_size[i],
                                counts, param, logs)
                })
        ...
        if gram_length == 3:
            ...
            gram_frequency = [[{
                '_'.join([gram1, gram2]):
                smooth_freq(gram_count, self.trigram_totals[i], vocab_size[i],
                            counts, param, logs)
                for gram2, gram_count in self.trigram_counts[i].get(
                    gram1).items()
            } for gram1 in self.trigram_counts[i].keys()] for i in [0, 1]]
            ...
            for i in [0, 1]:
                gram_frequency[i].update({
                    '<unk>':
                    smooth_freq(1, self.trigram_totals[i], vocab_size[i],
                                counts, param, logs)
                })
    ...
    def _comment_lengths_(self, comments):
        ...
        comment_lengths = [
            Counter(
                [len(comment[1]) for comment in comments if comment[2] == i])
            for i in range(2)
        ]
        comment_lengths = [
            zip(comment_lengths[0].keys(), comment_lengths[0].values()),
            zip(comment_lengths[1].keys(), comment_lengths[1].values())
        ]
        self.sorted_comment_lengths = [
            sorted(comment_lengths[0], key=lambda x: x[0]),
    ...
    def word_frequency_graphs(self, gram_length=1, min_rank=0, max_rank=25):
        ...
        if gram_length == 1:
            gram_freq = self._additive_smoothing_(gram_length,
                                                  param=0,
                                                  counts=True,
                                                  logs=False)
            title = 'Unigram'
        ...
        neg_vocab_words, neg_vocab_freq = list(
            zip(*sorted(zip(gram_freq[0].keys(), gram_freq[0].values()),
                        key=lambda x: -x[1])[min_rank:max_rank]))
        pos_vocab_words, pos_vocab_freq = list(
            zip(*sorted(zip(gram_freq[1].keys(), gram_freq[1].values()),
                        key=lambda x: -x[1])[min_rank:max_rank]))
        ...
        hover_1 = HoverTool(
            tooltips=[('Word', "$neg_vocab_words"), ('Freq',
                                                     '$neg_vocab_freq')])
        hover_2 = HoverTool(
            tooltips=[('Word', "$pos_vocab_words"), ('Freq',
                                                     '$pos_vocab_freq')])

        p1 = figure(x_range=neg_vocab_words,
                    plot_height=300,
                    plot_width=750,
                    tools=[hover_1],
                    title="Appropriate Question {} Counts".format(title))
        p2 = figure(x_range=pos_vocab_words,
                    plot_height=300,
                    plot_width=750,
                    tools=[hover_2],
                    title="Inappropriate Question {} Counts".format(title))

        p1.vbar(x='neg_vocab_words',
                top='neg_vocab_freq',
                width=0.9,
                source=source)
        p2.vbar(x='pos_vocab_words',
                top='pos_vocab_freq',
                width=0.9,
                source=source)
    ...
    def _gram_differences_(self, gram_length=1):
        ...
        gram_freq_diffs = []
        for gram in vocab:
            gram_freq_diff = gram_freq[0].get(gram, 0) - gram_freq[1].get(
                gram, 0)
    ...
    def comment_length_graph(self):
        ...
        neg_len_freq_plotting = [
            neg_len_counts[neg_len.index(i)] /
            sum(neg_len_counts) if i in neg_len else 0
            for i in range(max_length + 1)
        ]
        pos_len_freq_plotting = [
            pos_len_counts[pos_len.index(i)] /
            sum(pos_len_counts) if i in pos_len else 0
            for i in range(max_length + 1)
        ]
        data = {
        ...
        p.vbar(x='lengths', top='pos', width=0.9, source=data,
               color=colors[1], alpha=0.75, legend='Inappropriate')
        p.vbar(x='lengths', top='neg', width=0.9, source=data,
               color=colors[0], alpha=0.75, legend='Appropriate')
