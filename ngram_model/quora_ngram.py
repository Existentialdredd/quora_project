...
class NgramModel(object):
    ...
    def good_turing(self, gram_length=1, param=0.5, logs=True):
        ...
        for i in [0, 1]:
            ...
            for val in gram_count_by_occur_count[i].keys():
                gram_smoothing_ratios[i][val] = (
                    gram_count_by_occur_count[i].get(
                        val + 1, param * gram_count_by_occur_count[i].get(val))
                    / gram_count_by_occur_count[i].get(val))

            for key, val in gram_counts[i].items():
                gram_frequency[i][key] = freq(
                    (val + 1) * gram_smoothing_ratios[i].get(val) / total)
    ...
    def _smoothed_gram_freqs_(self,
                              gram,
                              gram_models,
                              cnvx_param=0.5,
                              logs=True):
        ...
        num_tokens = len(gram.split('_'))
        class_gram_freq = [
            1 / sum(self.unigram_counts[i].values()) for i in [0, 1]
        ]
        unigrams = gram.split('_')

        convex_comb = lambda param, arg1, arg2: param * arg1 + (1 - param
                                                                ) * arg2

        if num_tokens >= 1:
            unigram_freq = [[
                convex_comb(cnvx_param,
                            self._gram_freq_(gram, gram_models)[i],
                            class_gram_freq[i]) for gram in unigrams
            ] for i in [0, 1]]
            gram_freq = [val for sublist in unigram_freq for val in sublist]

        if num_tokens >= 2:
            bigrams = [
                '_'.join(unigrams[i:i + 2]) for i in range(num_tokens - 1)
            ]

            unigram_freq_products = [[
                np.prod(unigram_freq[j][i:i + 2])
        ...
        if num_tokens == 3:
            ...
            trigram_freq = [
                convex_comb(cnvx_param,
                            ...
                            bigram_freq_products[i]) for i in [0, 1]
            ]
    ...
    def jelinek_mercer(self,
        ...
        if scnd_smoother == 'additive':
            ...
            for gram_model in range(1, gram_length + 1):
                gram_models.append(
                    self._additive_smoothing_(gram_model,
                                              param,
                                              counts=False,
                                              logs=False))
    ...
    def train_classifier(self, gram_length=1, smoothing='additive', **kwargs):
        ...
        if smoothing == 'additive':
            ...
            self.gram_frequency = self.jelinek_mercer(gram_length,
                                                      cnvx_param,
                                                      scnd_smoother,
                                                      param=smth_param)
        else:
    ...
    def predict(self, comment):
        ...
        **kwargs            parameters needed for the indicated smoothing method
        ...
        if self.gram_length == 1:
            log_probs = [
                sum([
                    self.gram_frequency[i].get(gram, unk_prob[i])
                    for gram in comment
                ]) for i in [0, 1]
            ]
        elif self.gram_length == 2:
            comment_grams = [
                '_'.join(comment[i:i + 2]) for i in range(len(comment) - 1)
            ]
            log_probs = [
                sum([
                    self.gram_frequency[i].get(gram, unk_prob[i])
            ...
            comment_grams = [
                '_'.join(comment[i:i + 3]) for i in range(len(comment) - 2)
            ]
            log_probs = [
                sum([
                    self.gram_frequency[i].get(gram, unk_prob[i])
    ...
    def evaluate_classifier(self, comments, labels, report=True, file=False):
        ...
        if report:
            print('------{}------\n{}'.format(
                "Confusion Matrix", confusion_matrix(labels,
                                                     predicted_labels)))
            print('------{}------\n{}'.format(
        ...
        if file:
            ...
            file_ext = ''.join([
                'NG-', self.smoothing_info['smoothing_method'], '-',
                str(self.gram_length), '-', now_time
            ])
