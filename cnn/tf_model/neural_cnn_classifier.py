...
class Neural_CNN(object):
    ...
    def __init__(self, setup):
        ...
        self.n_filters_per_kernel_by_lyr = setup.get(
            'n_filters_per_kernel_by_lyr', [10])
        self.kernel_sizes_by_lyr = setup.get('kernel_sizes_by_lyr',
                                             [[1, 2, 3]])
        ...
        self.root_log_dir = setup.get(
            'root_log_dir', ''.join([os.getcwd(), '/neural_cnn_logs']))
        self.check_pt_dir = setup.get(
            'checkpoint_dir', ''.join([os.getcwd(), '/neural_cnn_ckpts']))
        self.summary_dir = setup.get(
            'summary_dir', ''.join([os.getcwd(), '/neural_cnn_run_summary']))
        self.graph = None
    ...
    def _embedding_lookup_layer_(self, W_embed_, sequences_, reduce='sum'):
        ...
        """
        embedding_lkup_ = tf.nn.embedding_lookup(W_embed_,
                                                 sequences_,
                                                 name='Embeddings')
    ...
    def _hidden_layers_(self, embedding_layer_, training_):
        ...
        for i, hidden_size in enumerate(self.n_hidden):
            with tf.variable_scope(("FCLayer_%d" % i)):
                if self.BATCH_NORM:
                    fc_ = tf.layers.dense(fc_,
                                          hidden_size,
                                          kernel_initializer=he_init_,
                                          name=("Hidden_%d_b4_bn" % i))
                    fc_ = tf.layers.batch_normalization(fc_,
                                                        training=training_,
                                                        name=("Hidden_%d_bn" %
                                                              i))
                    fc_ = self.__activation_lookup__(fc_,
                                                     ("Hidden_%d_act" % i))
                else:
                    fc_ = tf.layers.dense(fc_,
                                          hidden_size,
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("EmbeddingLyr"):
                embedding_layer_ = self._embedding_lookup_layer_(
                    W_embed_, sequences_)
    ...
    def _file_names_(self):
        ...
        self.most_recent_summary_file = ''.join(
            [self.summary_dir, '/most_recent_summary.txt'])
