...
class QuoraTFDataset():
    ...
    def get_tensor_datasets(self) -> tf.data.Dataset:
        """
        """
        sequence_length = len(self.data.observations[0].token_ids)
        train_ds = tf.data.Dataset.from_generator(
            self.train_data.feature_dict_generator,
            output_signature={
                "token_ids": tf.TensorSpec(shape=(sequence_length), dtype=tf.int64),
                "token_types": tf.TensorSpec(shape=(sequence_length), dtype=tf.int64),
                "attention_mask": tf.TensorSpec(shape=(sequence_length), dtype=tf.int64),
