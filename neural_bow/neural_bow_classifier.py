...
class Neural_BOW(object):
    ...
    def _placeholders_(self):
        ...
        """
        sequences_ = tf.placeholder(tf.int32, shape=[None,self.MAX_SEQ_LEN],name='sequences')
        W_embed_ = tf.placeholder(tf.float32,shape=[self.N_TOKN+1,self.EMBD_DIM],name='W_embed')
    ...
    def _embedding_lookup_layer_(self,W_embed_,sequences_,reduce='sum'):
        ...
        RETURNS:
        xs_         (tf.tensor) vector representation of an order.
        """
        xe_ = tf.nn.embedding_lookup(W_embed_,sequences_,name='Embeddings')
        if reduce == 'sum':
            xs_ = tf.reduce_sum(xe_,axis=1,name='BagOfProducts')
        elif reduce == 'mean':
            xs_ = tf.reduce_mean(xe_,axis=1,name='BagOfProducts')
    ...
    def _fully_connected_layer_(self,xs_,training_):
        """
        ...
        for i,dim in enumerate(self.n_hidden):
            with tf.variable_scope(("HiddenLayer_%d"%i)):
                if self.BATCH_NORM:
                    ...
                    h_ = tf.layers.dense(h_,dim,activation=self.hidden_activation,
                                        kernel_initializer=he_init_,
                                        name=("Hidden_%d"%i))
                if self.DROP_RATE > 0:
                    h_ = tf.layers.dropout(h_,rate=self.DROP_RATE,training=training_)
        return h_
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("EmbeddingLyr"):
                xs_ = self._embedding_lookup_layer_(W_embed_,sequences_)

            with tf.name_scope("HiddenLyr"):
                h_ = self._fully_connected_layer_(xs_,training_)
